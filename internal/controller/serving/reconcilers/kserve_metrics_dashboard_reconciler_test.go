...
var _ = Describe("KserveMetricsDashboardReconciler", func() {
	...
	Describe("When deploying a Inference Service", func() {
		...
		When("Caikit Runtime is used", func() {
			It("should create ConfigMap with supported=true and Caikit metrics", func(ctx SpecContext) {
				// Create Caikit ServingRuntime
				servingRuntime := createServingRuntime("caikit-runtime", map[string]string{
					constants.KServeRuntimeAnnotation: constants.CaikitRuntimeName,
				})

				// Create InferenceService using Caikit runtime
				isvc := &kservev1beta1.InferenceService{
					ObjectMeta: metav1.ObjectMeta{
						Name:      "caikit-model",
						Namespace: "test-namespace",
					...
					Spec: kservev1beta1.InferenceServiceSpec{
						Predictor: kservev1beta1.PredictorSpec{
							Model: &kservev1beta1.ModelSpec{
								ModelFormat: kservev1beta1.ModelFormat{Name: "caikit"},
								Runtime:     ptr.To("caikit-runtime"),
							},
						},
					},
				}
				...
				Expect(configMap.Data["supported"]).To(Equal("true"))
				Expect(configMap.Data["metrics"]).To(Equal(finaldata))
				Expect(configMap.Data["metrics"]).To(ContainSubstring("caikit-model"))
				Expect(configMap.Data["metrics"]).To(ContainSubstring("test-namespace"))
			})
		})
